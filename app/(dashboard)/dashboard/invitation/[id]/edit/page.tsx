...
export default function InvitationEditPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    if (isFetching) return (
        <div className="min-h-[400px] flex flex-col items-center justify-center text-gray-400 gap-4 uppercase italic font-black tracking-[0.2em]">
            <Loader2 className="w-10 h-10 animate-spin text-blue-600" />
            <p className="text-[10px] animate-pulse">Sinkronisasi Data...</p>
        </div>
    );
                    ...
                    <div className="fixed inset-0 z-[150] flex items-center justify-center p-4 overflow-hidden">
                        ...
                        >
                            <div className="flex items-center gap-3 mb-4">
                                ...
                                <h3 className="text-[11px] font-black uppercase tracking-widest text-slate-900 leading-none">{modal.title}</h3>
                            </div>
                            <p className="text-[11px] text-gray-500 mb-8 font-medium italic italic leading-relaxed">{modal.message}</p>
                            <div className="flex justify-end gap-1">
                                {modal.type === 'confirm' ? (
                                    <>
                                        <button onClick={() => setModal({ ...modal, show: false })} className="px-4 py-2 text-[10px] font-bold uppercase text-gray-400 hover:text-slate-900 transition-colors tracking-widest">Batal</button>
                                        <button onClick={() => modal.onConfirm?.()} className="px-6 py-2 bg-blue-600 text-[10px] font-bold uppercase text-white rounded-sm transition-all tracking-widest">Lanjutkan</button>
                                    </>
                                ) : (
                                    <button onClick={() => setModal({ ...modal, show: false })} className={`px-6 py-2 ${modal.type === 'success' ? 'bg-emerald-600' : modal.type === 'error' ? 'bg-rose-600' : 'bg-slate-900'} text-[10px] font-black uppercase text-white rounded-sm transition-all tracking-widest`}>Mengerti</button>
                                )}
                            </div>
                        </motion.div>
                    </div>
                ...
                <div className="space-y-4">
                    ...
                    <div className="flex items-center gap-5">
                        ...
                        <div>
                            ...
                            <p className="text-[10px] text-gray-400 font-bold uppercase tracking-widest mt-2 bg-gray-50 px-3 py-1 border border-gray-100 rounded-sm inline-block">
                                Editing Invitation Archive: <span className="text-blue-600 italic">/{formData.slug}</span>
                            </p>
                        </div>
                    </div>
                </div>
                        ...
                        return (
                            ...
                            >
                                ...
                                <div className="text-left">
                                    <p className={`text-[10px] font-black tracking-[0.1em] uppercase ${isActive ? 'italic' : ''}`}>{step.title}</p>
                                </div>
                            </button>
                        );
                    })}
                ...
                <div className="lg:col-span-9 space-y-8">
                    <div className="bg-white border border-gray-200 rounded-sm shadow-xl min-h-[600px] flex flex-col overflow-hidden">
                                ...
                                <motion.div key={currentStep} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-10">
                                    <div className="border-b border-gray-100 pb-8 flex items-center justify-between">
                                        <div>
                                            ...
                                            <p className="text-[10px] text-gray-400 font-bold uppercase tracking-widest mt-2 italic">Refining operational data for {steps[currentStep].id} block.</p>
                                        </div>
                                            ...
                                            {themes.map((theme) => (
                                                <div key={theme.id} onClick={() => setFormData({ ...formData, themeId: theme.id })} className={`p-6 rounded-sm border-2 transition-all cursor-pointer flex items-center justify-between group ${formData.themeId === theme.id ? "bg-blue-50/30 border-blue-600" : "bg-white border-gray-100 hover:border-gray-300"}`}>
                                                    <div className="flex items-center gap-6">
                                                        ...
                                                        <div><p className="font-black text-sm uppercase tracking-tight text-slate-900 italic">{theme.name}</p></div>
                                                    </div>
                                    ...
                                    {currentStep === 1 && (
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-16">
                                            <div className="space-y-6">
                                                <h3 className="text-xs font-black uppercase tracking-[0.2em] text-slate-900 italic border-l-4 border-blue-600 pl-4">The Bride</h3>
                                                <div className="space-y-4">
                                                    <input value={formData.brideName} onChange={(e) => setFormData({ ...formData, brideName: e.target.value })} className="w-full bg-gray-50 border border-gray-200 rounded-sm px-5 py-3 text-xs font-black outline-none focus:border-blue-500" placeholder="Full Name" />
                                                    <ErrorMsg name="brideName" />
                                                    <input value={formData.brideShort} onChange={(e) => setFormData({ ...formData, brideShort: e.target.value })} className="w-full bg-gray-50 border border-gray-200 rounded-sm px-5 py-3 text-xs font-black outline-none focus:border-blue-500 italic" placeholder="Short Name (URL)" />
                                                    <ErrorMsg name="brideShort" />
                                                    <input value={formData.brideImage} onChange={(e) => setFormData({ ...formData, brideImage: e.target.value })} className="w-full bg-gray-50 border border-gray-200 rounded-sm px-5 py-3 text-xs font-mono outline-none focus:border-blue-500" placeholder="Avatar URL" />
                                                </div>
                                            </div>
                                            <div className="space-y-6">
                                                <h3 className="text-xs font-black uppercase tracking-[0.2em] text-slate-900 italic border-l-4 border-slate-900 pl-4">The Groom</h3>
                                                <div className="space-y-4">
                                                    <input value={formData.groomName} onChange={(e) => setFormData({ ...formData, groomName: e.target.value })} className="w-full bg-gray-50 border border-gray-200 rounded-sm px-5 py-3 text-xs font-black outline-none focus:border-blue-500" placeholder="Full Name" />
                                                    <ErrorMsg name="groomName" />
                                                    <input value={formData.groomShort} onChange={(e) => setFormData({ ...formData, groomShort: e.target.value })} className="w-full bg-gray-50 border border-gray-200 rounded-sm px-5 py-3 text-xs font-black outline-none focus:border-blue-500 italic" placeholder="Short Name (URL)" />
                                                    <ErrorMsg name="groomShort" />
                                                    <input value={formData.groomImage} onChange={(e) => setFormData({ ...formData, groomImage: e.target.value })} className="w-full bg-gray-50 border border-gray-200 rounded-sm px-5 py-3 text-xs font-mono outline-none focus:border-blue-500" placeholder="Avatar URL" />
                                                </div>
                                            </div>
                                    ...
                                    {currentStep === 2 && (
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                                            <div className="space-y-3">
                                                <label className="text-[10px] uppercase font-black text-gray-400 tracking-[0.3em]">Bride Parentals</label>
                                                <textarea value={formData.brideParents} onChange={(e) => setFormData({ ...formData, brideParents: e.target.value })} className="w-full bg-gray-50 border border-gray-200 rounded-sm px-6 py-5 text-sm h-40 outline-none focus:bg-white focus:border-blue-500 transition-all font-bold italic italic leading-relaxed" />
                                            </div>
                                            <div className="space-y-3">
                                                <label className="text-[10px] uppercase font-black text-gray-400 tracking-[0.3em]">Groom Parentals</label>
                                                <textarea value={formData.groomParents} onChange={(e) => setFormData({ ...formData, groomParents: e.target.value })} className="w-full bg-gray-50 border border-gray-200 rounded-sm px-6 py-5 text-sm h-40 outline-none focus:bg-white focus:border-blue-500 transition-all font-bold italic italic leading-relaxed" />
                                            </div>
                                        </div>
                                    )}
                                    ...
                                    {currentStep === 3 && (
                                        <div className="space-y-12">
                                            <div className="bg-gray-50 p-8 rounded-sm border border-gray-100 space-y-4">
                                                <p className="text-[10px] font-black text-blue-600 uppercase tracking-widest mb-2 italic">Akad Celebration</p>
                                                <input type="date" value={formData.akadDate} onChange={(e) => setFormData({ ...formData, akadDate: e.target.value })} className="w-full bg-white border border-gray-200 rounded-sm px-5 py-3 text-xs outline-none focus:border-blue-500 transition-all" />
                                                <input value={formData.akadPlace} onChange={(e) => setFormData({ ...formData, akadPlace: e.target.value })} className="w-full bg-white border border-gray-200 rounded-sm px-5 py-3 text-xs font-black italic outline-none focus:border-blue-500 transition-all" placeholder="Venue Name" />
                                                <textarea value={formData.akadAddress} onChange={(e) => setFormData({ ...formData, akadAddress: e.target.value })} className="w-full bg-white border border-gray-200 rounded-sm px-5 py-3 text-xs h-24 outline-none italic focus:border-blue-500 transition-all" placeholder="Full Address" />
                                                <input value={formData.akadMaps} onChange={(e) => setFormData({ ...formData, akadMaps: e.target.value })} className="w-full bg-white border border-gray-200 rounded-sm px-5 py-3 text-xs outline-none italic focus:border-blue-500 transition-all" placeholder="Google Maps URL (https://...)" />
                                            </div>
                                            <div className="bg-gray-50 p-8 rounded-sm border border-gray-100 space-y-4">
                                                <p className="text-[10px] font-black text-slate-900 uppercase tracking-widest mb-2 italic">Resepsi Wedding</p>
                                                <input type="date" value={formData.resepsiDate} onChange={(e) => setFormData({ ...formData, resepsiDate: e.target.value })} className="w-full bg-white border border-gray-200 rounded-sm px-5 py-3 text-xs outline-none focus:border-blue-500 transition-all" />
                                                <input value={formData.resepsiPlace} onChange={(e) => setFormData({ ...formData, resepsiPlace: e.target.value })} className="w-full bg-white border border-gray-200 rounded-sm px-5 py-3 text-xs font-black italic outline-none focus:border-blue-500 transition-all" placeholder="Venue Name" />
                                                <textarea value={formData.resepsiAddress} onChange={(e) => setFormData({ ...formData, resepsiAddress: e.target.value })} className="w-full bg-white border border-gray-200 rounded-sm px-5 py-3 text-xs h-24 outline-none italic focus:border-blue-500 transition-all" placeholder="Full Address" />
                                                <input value={formData.resepsiMaps} onChange={(e) => setFormData({ ...formData, resepsiMaps: e.target.value })} className="w-full bg-white border border-gray-200 rounded-sm px-5 py-3 text-xs outline-none italic focus:border-blue-500 transition-all" placeholder="Google Maps URL (https://...)" />
                                            </div>
                                        </div>
                                    )}
                                    ...
                                    {currentStep === 4 && (
                                        <div className="space-y-10">
                                            <div className="flex gap-3 bg-gray-50 p-6 rounded-sm border">
                                                <input id="galInput" className="bg-white border rounded-sm px-5 py-3 text-xs flex-1 outline-none font-mono italic" placeholder="Paste Image URL..." />
                                                <button onClick={() => { const el = document.getElementById('galInput') as HTMLInputElement; if (el.value) addGalleryItem(el.value); el.value = ''; }} className="inline-flex items-center gap-2 px-8 py-3 bg-blue-600 text-white rounded-sm text-[10px] font-black uppercase tracking-widest active:scale-95 transition-all">Import</button>
                                            </div>
                                            ...
                                            <div className="space-y-6">
                                                {getStory().map((item: any, i: number) => (
                                                    <div key={i} className="p-8 bg-gray-50 border border-gray-200 rounded-sm flex flex-col md:flex-row gap-10 relative group transition-all hover:bg-white">
                                                        <button onClick={() => removeStory(i)} className="absolute top-4 right-4 text-gray-400 hover:text-rose-600 font-black text-[9px] uppercase tracking-widest flex items-center gap-1"><X size={12} strokeWidth={4} /> REMOVE</button>
                                                        <div className="w-10 md:w-44 h-44 rounded-sm bg-white shrink-0 overflow-hidden border border-gray-200 p-2">
                                                            {item.image ? <img src={fixImageUrl(item.image)} className="w-full h-full object-cover rounded-sm" /> : <div className="w-full h-full flex flex-col items-center justify-center text-gray-200"><ImageIcon size={40} /><span className="text-[8px] font-black mt-2">NO PREVIEW</span></div>}
                                                        </div>
                                                        <div className="flex-1 space-y-4">
                                                            <div className="grid grid-cols-2 gap-4">
                                                                <input value={item.image} onChange={(e) => updateStory(i, "image", e.target.value)} className="w-full bg-white border border-gray-200 rounded-sm px-4 py-2 text-xs font-mono" placeholder="Image URL" />
                                                                <input value={item.date} onChange={(e) => updateStory(i, "date", e.target.value)} className="w-full bg-white border border-gray-200 rounded-sm px-4 py-2 text-xs font-black uppercase tracking-widest" placeholder="Timeline Marker" />
                                                            </div>
                                                            <input value={item.title} onChange={(e) => updateStory(i, "title", e.target.value)} className="w-full bg-white border border-gray-200 rounded-sm px-4 py-2 text-sm font-black uppercase italic" placeholder="Event Designation" />
                                                            <textarea value={item.description} onChange={(e) => updateStory(i, "description", e.target.value)} className="w-full bg-white border border-gray-200 rounded-sm px-4 py-2 text-xs h-24 outline-none resize-none leading-relaxed font-bold italic italic" placeholder="Narrative Matrix Content..." />
                                                        </div>
                                                    </div>
                                                ))}
                                            </div>
                                    ...
                                    {currentStep === 6 && (
                                        <div className="p-8 bg-gray-50 border border-gray-200 rounded-sm space-y-8">
                                            <div className="space-y-4">
                                                <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Audio Master Source (MP3 / YouTube)</label>
                                                <input value={formData.musicUrl} onChange={(e) => setFormData({ ...formData, musicUrl: e.target.value })} className="w-full bg-white border border-gray-200 rounded-sm px-5 py-4 text-xs font-mono outline-none focus:border-blue-500" placeholder="PASTE URL SOURCE..." />
                                            </div>
                                            {formData.musicUrl && <div className="p-10 bg-white rounded-sm border flex items-center justify-center italic font-black text-[11px] text-blue-600 animate-pulse tracking-widest uppercase">Streaming Core Active & Placeholder</div>}
                                        </div>
                                    )}
                                    ...
                                    {currentStep === 7 && (
                                        <div className="space-y-10">
                                            <div className="flex items-center justify-between border-b pb-6">
                                                ...
                                                <button onClick={addBank} className="inline-flex items-center gap-2 px-8 py-3 bg-emerald-600 hover:bg-emerald-700 text-white rounded-sm text-[10px] font-black uppercase tracking-widest transition-all"><PlusCircle size={16} className="inline mr-2" /> Add Account</button>
                                            </div>
                        ...
                        <div className="bg-white border-t border-gray-100 p-8 flex items-center justify-between mt-auto">
                            <button onClick={prevStep} disabled={currentStep <= 1} className={`inline-flex items-center gap-3 px-8 py-3 rounded-sm text-[10px] font-black uppercase tracking-widest transition-all border border-gray-200 bg-white ${currentStep <= 1 ? "opacity-0 invisible pointer-events-none" : "text-gray-500 hover:text-slate-900"}`}>
                                <ChevronLeft size={18} strokeWidth={3} /> Previous Stage
                            </button>
                             <div className="flex gap-3">
                                <button onClick={() => handleSubmit()} disabled={loading} className="inline-flex items-center gap-3 px-8 py-3 bg-white border border-gray-200 text-slate-400 hover:text-emerald-600 hover:border-emerald-600 rounded-sm font-black text-[10px] tracking-widest uppercase transition-all">
                                    {loading ? <Loader2 size={16} className="animate-spin" /> : <Save size={16} strokeWidth={3} />} Save
                                </button>
                                {currentStep === steps.length - 1 ? (
                                    <button onClick={() => handleSubmit(true)} disabled={loading} className="inline-flex items-center gap-3 px-8 py-3 bg-white border-2 border-emerald-600 text-emerald-600 hover:bg-emerald-50 rounded-sm font-black text-[10px] tracking-widest uppercase transition-all">
                                        {loading ? <Loader2 size={16} className="animate-spin" /> : <Save size={16} strokeWidth={4} />} Finish Editing
                                    </button>
                                ) : (
                                    <button onClick={nextStep} className="inline-flex items-center gap-3 px-8 py-3 bg-white border-2 border-blue-600 text-blue-600 hover:bg-blue-50 rounded-sm font-black text-[10px] tracking-widest uppercase transition-all italic">
                    ...
                    <div className="text-center text-[9px] font-black text-gray-300 uppercase tracking-[0.5em] italic">Engine Ready: V2.3.0-Editor_Partition</div>
                </div>
            </div>
        </div>
    );
